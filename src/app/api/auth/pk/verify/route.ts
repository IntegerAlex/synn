...
export async function POST(request: Request) {
  try {
    ...
    if (!body?.challengeId || !body?.signature) {
      return NextResponse.json({ error: { code: 'INVALID_REQUEST', message: 'Missing challengeId or signature' } }, { status: 400 });
    }
    ...
    if (!result.ok) {
      ...
      return NextResponse.json(
        { error: { code: reason, message: 'Signature invalid or challenge expired' } },
        { status: 401 },
      );
    }
    ...
    return NextResponse.json(
      { error: { code: 'PK_VERIFY_ERROR', message: error?.message || 'Failed to verify signature' } },
      { status: 500 },
    );
  }
}
