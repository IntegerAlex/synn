...
export async function POST(request: NextRequest) {
  try {
    ...
    const { repoFullName, viewState, title, description, expiresInHours } = body;

    // Input validation
    if (!repoFullName || typeof repoFullName !== 'string') {
      return NextResponse.json(
        { error: { code: 'INVALID_INPUT', message: 'repoFullName is required and must be a string' } },
        { status: 400 }
      );
    }
    ...
    if (!/^[a-zA-Z0-9._-]+\/[a-zA-Z0-9._-]+$/.test(repoFullName)) {
      return NextResponse.json(
        { error: { code: 'INVALID_INPUT', message: 'Invalid repository format. Expected: owner/repo' } },
        { status: 400 }
      );
    }
    ...
    if (!viewState || typeof viewState !== 'object' || Array.isArray(viewState)) {
      return NextResponse.json(
        { error: { code: 'INVALID_INPUT', message: 'viewState is required and must be an object' } },
        { status: 400 }
      );
    }

    // Validate viewState structure to prevent injection
    const allowedKeys = ['branch', 'selectedCommit', 'graphFilters', 'graphLimit'];
    const viewStateKeys = Object.keys(viewState);
    const invalidKeys = viewStateKeys.filter(key => !allowedKeys.includes(key));
    if (invalidKeys.length > 0) {
      return NextResponse.json(
        { error: { code: 'INVALID_INPUT', message: `Invalid viewState keys: ${invalidKeys.join(', ')}` } },
        { status: 400 }
      );
    }
    ...
    if (viewState.graphFilters) {
      if (typeof viewState.graphFilters !== 'object' || Array.isArray(viewState.graphFilters)) {
        return NextResponse.json(
          { error: { code: 'INVALID_INPUT', message: 'graphFilters must be an object' } },
          { status: 400 }
        );
      }
      const allowedFilterKeys = ['showMergeCommits', 'showTags', 'highlightedBranches'];
      ...
      if (invalidFilterKeys.length > 0) {
        return NextResponse.json(
          { error: { code: 'INVALID_INPUT', message: `Invalid graphFilters keys: ${invalidFilterKeys.join(', ')}` } },
          { status: 400 }
        );
      }
      // Validate highlightedBranches is an array
      if (viewState.graphFilters.highlightedBranches && !Array.isArray(viewState.graphFilters.highlightedBranches)) {
        return NextResponse.json(
          { error: { code: 'INVALID_INPUT', message: 'highlightedBranches must be an array' } },
          { status: 400 }
        );
      }
    }
    ...
    if (viewState.graphLimit !== undefined) {
      ...
      if (!Number.isFinite(limit) || limit < 1 || limit > 10000) {
        return NextResponse.json(
          { error: { code: 'INVALID_INPUT', message: 'graphLimit must be between 1 and 10000' } },
          { status: 400 }
        );
      }
    }
    ...
    if (expiresInHours !== undefined) {
      ...
      if (!Number.isFinite(hours) || hours < 0.1 || hours > 8760) {
        ...
        return NextResponse.json(
          { error: { code: 'INVALID_INPUT', message: 'expiresInHours must be between 0.1 and 8760' } },
          { status: 400 }
        );
      }
    }
    ...
    if (clerkUserId) {
      ...
      if (!validation.valid) {
        return NextResponse.json(
          { error: { code: 'ACCESS_DENIED', message: 'You do not have access to this repository' } },
          { status: 403 }
        );
      }
      ...
      try {
        ...
        return NextResponse.json(
          { error: { code: 'ACCESS_DENIED', message: 'Cannot access this repository. It may be private or you may not have permission.' } },
          { status: 403 }
        );
      }
    } else {
      ...
      return NextResponse.json(
        { error: { code: 'UNAUTHORIZED', message: 'Authentication required to create shares' } },
        { status: 401 }
      );
    }
