...
export async function GET(
  ...
  try {
    ...
    if (!id || id.length < 16 || id.length > 32 || !/^[A-Za-z0-9_-]+$/.test(id)) {
      return NextResponse.json(
        { error: { code: 'INVALID_SHARE_ID', message: 'Invalid share ID format' } },
        { status: 400 }
      );
    }
    ...
    if (!hasAccess) {
      if (!creatorClerkUserId) {
        ...
        return NextResponse.json(
          { error: { code: 'AUTH_REQUIRED', message: 'Authentication required to view this shared repository' } },
          { status: 401 }
        );
      }
      ...
      try {
        const creatorService = await getGitHubServiceForUser(creatorClerkUserId, sharedView.repoFullName);
