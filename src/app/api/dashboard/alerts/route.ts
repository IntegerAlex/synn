...
export async function GET(request: Request) {
  try {
    ...
    const unauthorized = logs.filter((log) => log.responseStatus === 401 || log.responseStatus === 403).length;
    const errors = logs.filter((log) => log.responseStatus && log.responseStatus >= 400).length;
    ...
    return NextResponse.json(
      { error: { code: 'DASHBOARD_ALERTS_ERROR', message: error?.message || 'Failed to load alerts' } },
      { status: 500 },
    );
  }
}
