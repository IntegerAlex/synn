...
export function DashboardClient() {
  ...
  const [chartFocus, setChartFocus] = useState<{ endpoint?: string; user?: string } | null>(null);
  ...
  const exportCsv = useCallback((rows: Record<string, any>[], fileName: string) => {
  ...
  const usageByEndpoint = usageQuery.data?.endpoints ?? [];
  const topUsers = useMemo(() => {
    if (!usageQuery.data?.users) return [];
    const grouped = new Map<string, { clerkUserId: string | null; userId: number | null; total: number; endpoints: { endpoint: string; total: number }[] }>();
    for (const row of usageQuery.data.users) {
      const key = `${row.clerkUserId || 'unknown'}-${row.userId ?? 'none'}`;
      if (!grouped.has(key)) {
        grouped.set(key, { clerkUserId: row.clerkUserId, userId: row.userId, total: 0, endpoints: [] });
      }
  ...
  const isLoadingAny = logsQuery.isLoading || usageQuery.isLoading || alertsQuery.isLoading;
  ...
  const navItems: { id: SectionKey; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'api', label: 'API Calls', icon: <TableProperties className="w-4 h-4" /> },
    { id: 'users', label: 'Users', icon: <Network className="w-4 h-4" /> },
    { id: 'alerts', label: 'Alerts', icon: <AlertTriangle className="w-4 h-4" /> },
    { id: 'controls', label: 'Controls', icon: <ShieldOff className="w-4 h-4" /> },
  ];
    ...
    <div className="min-h-screen bg-linear-to-br from-[#0b1020] via-[#0d1117] to-[#0b0f14] text-white flex">
      ...
      >
        <div className="px-5 py-6 border-b border-[#30363d]">
          ...
          {!sidebarCollapsed && <div className="text-xs text-gray-400 mt-1">Secure overview</div>}
        </div>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg transition-colors ${
                  isActive ? 'bg-[#111820] text-white' : 'text-gray-300 hover:bg-[#111820]'
                }`}
              >
                {item.icon}
        ...
        <div className="sticky top-0 z-10 backdrop-blur bg-[#0d1117]/85 border-b border-[#1f2a38] px-4 sm:px-6 py-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <h1 className="text-2xl font-semibold mt-1">Security & Audit Dashboard</h1>
              <p className="text-sm text-gray-400">Password gated · Live insights · Responsive</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Chip icon={<Network className="w-3.5 h-3.5" />} label="Live data" tone="neutral" />
              <Chip
              ...
              >
                {isLoadingAny ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />}
                Refresh data
              </button>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-6">
          ...
          {activeSection === 'overview' && (
            <SectionCard id="charts" padded>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-stretch">
                <ChartCard
                  title="By endpoint"
                  valueLabel={endpointUsage.length ? `${formatNumber(endpointUsage.length)} endpoints` : 'No data'}
                >
                  <DonutChart
                    data={endpointUsage.map((d) => ({ label: d.endpoint, total: Number(d.total) }))}
                ...
                <ChartCard
                  title="By user"
                  valueLabel={activeUsers ? `${formatNumber(activeUsers)} users` : 'No data'}
                >
                  <DonutChart
                    data={userUsage.map((u) => ({ label: u.label, total: Number(u.total) }))}
                ...
                <div className="rounded-xl border border-[#1f2a38] bg-[#0b0f14]/80 p-4 space-y-4 h-full">
                  <h4 className="text-sm font-semibold text-gray-200">Top endpoints</h4>
                  <div className="space-y-2">
                    {endpointUsage.slice(0, 8).map((row, idx) => (
                      <div key={row.endpoint} className="flex items-center justify-between text-sm">
                        <div className="flex items-center gap-2">
                          <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: donutColor(idx) }} />
                          <span className="text-gray-300">{row.endpoint}</span>
                        </div>
                        <span className="text-gray-100 font-semibold tabular-nums">{formatNumber(row.total)}</span>
                      </div>
                    ))}
                    {endpointUsage.length === 0 && <EmptyState message="No usage data yet" />}
                  </div>
                  <h4 className="text-sm font-semibold text-gray-200 pt-2 border-t border-[#1f2a38]">Top users</h4>
                  <div className="space-y-2">
                    {topUsers.slice(0, 8).map((user, idx) => (
                      <div key={`${user.clerkUserId}-${user.userId}-${idx}`} className="flex items-center justify-between text-sm">
                        <div className="flex items-center gap-2">
                          <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: donutColor(idx) }} />
                          <span className="text-gray-300">{user.clerkUserId || `user-${user.userId ?? 'n/a'}`}</span>
                        </div>
                        <span className="text-gray-100 font-semibold tabular-nums">{formatNumber(user.total)}</span>
                      </div>
                    ))}
                    {topUsers.length === 0 && <EmptyState message="No user usage yet" />}
                  </div>
                </div>
              </div>
            </SectionCard>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-[#0b0f14] border-b border-[#1f2a38]">
                  <tr>
                    {['Time', 'User', 'Activity', 'IP', 'Repo', 'Status'].map((col) => (
                      <th key={col} className="px-4 py-3 text-left font-semibold text-gray-300">
                        {col}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {activityLogs.map((log) => (
                    <tr key={log.id} className="border-b border-[#161c24] hover:bg-[#111820]/70">
                      ...
                      <td className="px-4 py-3">
                        ...
                        <div className="text-xs text-gray-400">{log.category ?? 'activity'}</div>
                      </td>
                      <td className="px-4 py-3">
                        <div className="font-semibold">{log.activityType}</div>
                        {log.requestPath && (
                          <div className="text-xs text-gray-400">{log.requestMethod} {log.requestPath}</div>
                        )}
                      </td>
                      <td className="px-4 py-3 font-mono text-xs text-gray-400">
                        {log.ipAddress || 'encrypted'}
                      </td>
                      <td className="px-4 py-3 text-gray-300">{log.repoFullName || '—'}</td>
                      <td className="px-4 py-3">
                        {log.responseStatus ? (
                          <span
                  ...
                  {activityLogs.length === 0 && (
                    <tr>
                      <td className="px-4 py-6 text-center text-gray-500" colSpan={6}>
                        {logsQuery.isLoading ? <LoadingRow /> : <EmptyState message="No activity in this window" />}
                      </td>
                    </tr>
                  )}
                </tbody>
          ...
          <SectionCard id="api-calls">
              ...
              action={
                <div className="flex gap-2">
                  ...
                  <PaginationControls
                    ...
                    disableNext={!logsQuery.data?.apiRequests.pagination.hasMore}
                  />
                </div>
              }
            />
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-[#0b0f14] border-b border-[#1f2a38]">
                  <tr>
                    {['Time', 'User', 'Method', 'Path', 'Status', 'Latency'].map((col) => (
                      <th key={col} className="px-4 py-3 text-left font-semibold text-gray-300">
                        {col}
                      </th>
                    ))}
                ...
                <tbody>
                  {apiRequests.map((req) => (
                    <tr key={req.id} className="border-b border-[#161c24] hover:bg-[#111820]/70">
                      <td className="px-4 py-3 text-gray-300">
                        {new Date(req.createdAt).toLocaleString()}
                      </td>
                      <td className="px-4 py-3">
                        <div className="font-medium">{req.user?.email || `User ${req.userId ?? 'n/a'}`}</div>
                        <div className="text-xs text-gray-400">ID: {req.userId ?? '—'}</div>
                      </td>
                      <td className="px-4 py-3 font-semibold">{req.method}</td>
                  ...
                  {apiRequests.length === 0 && (
                    <tr>
                      <td className="px-4 py-6 text-center text-gray-500" colSpan={6}>
                        {logsQuery.isLoading ? <LoadingRow /> : <EmptyState message="No API calls in this window" />}
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            <HeaderRow
              ...
              action={
                <span className="text-xs text-gray-400 flex items-center gap-2">
                  ...
                  {alertsQuery.isLoading ? 'Scanning…' : `${suspicious.length} findings`}
                </span>
              }
            />
            {suspicious.length === 0 ? (
                ...
                {suspicious.map((alert, idx) => (
                  ...
                  >
                    ...
                    <div className="text-xs text-gray-500 mt-2">
                      Events: {alert.count} · Logs: {alert.logIds.slice(0, 5).join(', ')}
                      {alert.logIds.length > 5 ? ` +${alert.logIds.length - 5}` : ''}
                    </div>
                  </div>
                ))}
              </div>
          ...
          />
              <div className="space-y-2">
                    ...
                    >
                      ...
                      >
                        <div className="flex flex-col gap-1">
                          <span className="font-semibold text-gray-100">{user.email || user.clerkUserId}</span>
                          <span className="text-xs text-gray-400">
                            {user.name || 'Unnamed'} · repos: {formatNumber(user.repoCount)}
                          </span>
                        </div>
                        <span className="text-xs text-gray-400">
                      ...
                      {isExpanded && (
                        <div className="mt-3 space-y-1">
                          {user.repos.length === 0 && <EmptyState message="No repos" />}
                          {user.repos.map((repo) => (
                            <div
                              key={repo.id}
                            ...
                            >
                              <div className="flex flex-col">
                                <span className="text-gray-100">{repo.fullName || 'unknown'}</span>
                                <span className="text-xs text-gray-500">
                                  Stars: {formatNumber(repo.starsCount || 0)} · Forks: {formatNumber(repo.forksCount || 0)}
                                </span>
                              </div>
                              <span className="text-xs text-gray-500">{repo.isPrivate ? 'Private' : 'Public'}</span>
                            </div>
                          ))}
                        </div>
                      )}
                ...
                {usersQuery.data?.users.length === 0 && !usersQuery.isLoading && <EmptyState message="No users found" />}
              </div>
            </SectionCard>
          )}
    ...
    <div className="rounded-xl border border-[#1f2a38] bg-[#0f131a]/70 p-4 shadow-lg shadow-black/20 backdrop-blur h-full">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <div className="text-3xl font-semibold mt-2 tabular-nums">{formatNumber(value)}</div>
...
function SeverityBadge({ severity }: { severity: SuspiciousActivity['severity'] }) {
  const style = {
  ...
  return <span className={`px-2 py-1 rounded text-xs font-semibold ${style}`}>{severity}</span>;
}

function ControlCard({ title, description }: { title: string; description: string }) {
  return (
...
function Chip({ icon, label, tone }: { icon: React.ReactNode; label: string; tone?: 'neutral' | 'muted' }) {
  const styles =
    tone === 'neutral'
      ? 'bg-[#111820] text-gray-200 border border-[#1f2a38]'
      : 'bg-[#0f131a] text-gray-300 border border-[#1f2a38]';
  return (
    <span className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs ${styles}`}>
      {icon}
      {label}
    </span>
  );
    ...
    <div className="space-y-4">
      <div className="w-full flex items-center justify-center">
        <svg viewBox="0 0 36 36" className="w-48 h-48">
          <circle cx="18" cy="18" r="16" fill="transparent" stroke="#111820" strokeWidth="4" />
          ...
          <text x="18" y="17" textAnchor="middle" dominantBaseline="central" fill="white" fontSize="5" fontWeight="700">
            {formatNumber(active.value)}
          </text>
          <text x="18" y="22" textAnchor="middle" dominantBaseline="central" fill="#9CA3AF" fontSize="2.6">
            {activePercent}%
          </text>
        </svg>
      </div>
        ...
        {segments.map((seg, idx) => (
          ...
          >
            <span className="flex items-center gap-2">
              ...
              <span className="truncate max-w-[180px] text-left">{seg.label}</span>
            </span>
...
function ChartCard({ title, valueLabel, children }: { title: string; valueLabel?: string; children: React.ReactNode }) {
  return (
...
function donutColor(idx: number) {
  const palette = ['#7C3AED', '#22C55E', '#FACC15', '#38BDF8', '#F97316', '#E11D48', '#14B8A6', '#A855F7'];
