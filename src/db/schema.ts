...
export const reposTable = pgTable(
  ...
  (table) => ({
    ...
    userRepoUnique: index("repos_user_repo_unique").on(
      table.userId,
      table.githubRepoId,
    ),
  }),
...
export const fingerprintsTable = pgTable(
  "fingerprints",
  {
    id: integer().primaryKey().generatedAlwaysAsIdentity(),
...
export const activityLogsTable = pgTable(
  "activity_logs",
  {
    ...
    fingerprintId: integer().references(() => fingerprintsTable.id, {
      onDelete: "set null",
    }),
    // Activity type
  ...
  (table) => ({
    ...
    fingerprintIdIdx: index("activity_logs_fingerprint_id_idx").on(
      table.fingerprintId,
    ),
    ...
    repoFullNameIdx: index("activity_logs_repo_full_name_idx").on(
      table.repoFullName,
    ),
  }),
);
...
export const apiRequestsTable = pgTable(
  ...
  (table) => ({
    ...
    fingerprintIdIdx: index("api_requests_fingerprint_id_idx").on(
      table.fingerprintId,
    ),
...
export const githubApiUsageTable = pgTable(
  "github_api_usage",
  {
    id: integer().primaryKey().generatedAlwaysAsIdentity(),
  ...
  (table) => ({
    ...
    usageBucketUnique: uniqueIndex("github_api_usage_bucket_unique").on(
      table.clerkUserId,
      table.endpoint,
      table.bucketDate,
    ),
...
export const commitsTable = pgTable(
  ...
  (table) => ({
    ...
    userRepoHashUnique: index("commits_user_repo_hash_unique").on(
      table.userId,
      table.repoId,
      table.hash,
    ),
...
export const contributionsTable = pgTable(
  ...
  (table) => ({
    ...
    contributionDateIdx: index("contributions_contribution_date_idx").on(
      table.contributionDate,
    ),
    // Unique constraint: one row per user per repo per date
    userRepoDateUnique: index("contributions_user_repo_date_unique").on(
      table.userId,
      table.repoId,
      table.contributionDate,
    ),
    // Index for querying by user and date range
    userDateIdx: index("contributions_user_date_idx").on(
      table.userId,
      table.contributionDate,
    ),
  }),
...
export const sharedViewsTable = pgTable(
  "shared_views",
  {
    id: integer().primaryKey().generatedAlwaysAsIdentity(),
