...
export function CommitDetails() {
  ...
  const displayDiff = useMemo(() => {
    ...
    if (selectedFile) {
      ...
      for (const [filePath, diff] of fileDiffs.entries()) {
        if (
          filePath.endsWith(selectedFile) ||
  ...
  if (!selectedHash) {
    return (
      ...
      >
        <div
          className="flex-1 flex items-center justify-center text-text-sub text-sm"
          role="status"
          aria-live="polite"
        >
        ...
        <div className="p-4 border-b border-border-main">
          <div className="flex items-center gap-2 mb-2">
            ...
            <button
              type="button"
              onClick={() =>
                copyToClipboard(details.hash, "Copied commit hash")
              }
          ...
          <div className="flex items-start justify-between gap-2 mb-2">
            ...
            <button
              type="button"
              onClick={() =>
                copyToClipboard(details.message, "Copied commit message")
              }
          ...
          {details.body && (
            <p className="text-xs text-text-sub whitespace-pre-wrap">
              {details.body}
            </p>
          )}
        </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4 text-sm">
              ...
              <span className="text-text-sub">
                {details.stats.totalFiles} files
              </span>
            </div>
            {details.diff && (
              <button
            ...
            right={
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  aria-label={
                    showFullDiff ? "Show diff summary" : "Show full diff"
                  }
                >
...
function renderDiff(
  ...
  const highlighted = highlightUnifiedDiffLines(
    displayLines.join("\n"),
    filePathHint,
  );
    ...
    <>
      ...
      {hasMore && (
        <div className="text-gray-500 italic mt-2">
          ... ({lines.length - 50} more lines, click "Show Full Diff" to see
          all)
        </div>
      )}
    </>
