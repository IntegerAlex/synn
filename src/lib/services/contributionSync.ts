...
import { db } from "@/db";
import { commitsTable, contributionsTable, reposTable } from "@/db/schema";
...
function validateSyncOptions(options: SyncOptions): void {
  ...
  if (
    !options.clerkUserId ||
    typeof options.clerkUserId !== "string" ||
    options.clerkUserId.trim().length === 0
  ) {
  ...
  if (
    ...
    options.githubToken.trim().length === 0
  ) {
    throw new Error("Invalid githubToken: must be a non-empty string");
  }
  if (options.repoFullName !== undefined) {
    if (
      typeof options.repoFullName !== "string" ||
      options.repoFullName.trim().length === 0
    ) {
      throw new Error(
        "Invalid repoFullName: must be a non-empty string if provided",
      );
    }
    // Validate format: owner/repo
    if (
      !options.repoFullName.includes("/") ||
      options.repoFullName.split("/").length !== 2
    ) {
    ...
    const commitsChunk = nodeChunk
      .map((node) => {
        ...
        if (
          ...
          logger.warn("Skipping commit with invalid hash", {
        ...
        if (!node.date) {
          logger.warn("Skipping commit with invalid date", {
            hash: node.hash,
            repoId,
            userId,
          });
        ...
        const authorParts = node.author?.split("<") || [
          node.author || "Unknown",
        ];
        ...
        if (hash.length === 0 || hash.length > 40) {
          logger.warn("Skipping commit with invalid hash length", {
            hash,
            repoId,
            userId,
          });
        ...
        // Validate shortHash
        const shortHash = (node.shortHash || hash.substring(0, 7)).substring(
          0,
          7,
        );
...
async function syncRepoCommits(
  ...
  try {
    ...
    const graphData = await retry(() => githubService.getGraph(limit), {
      maxRetries: 3,
      initialDelayMs: 1000,
    ...
    for await (const commitsChunk of processCommitsInChunks(
      graphData,
      userId,
      repoId,
      1000,
      ...
      await db.transaction(async (tx) => {
        ...
        for (let i = 0; i < commitsChunk.length; i += batchSize) {
          ...
          try {
            ...
            await retry(() => tx.insert(commitsTable).values(batch), {
              maxRetries: 3,
              initialDelayMs: 500,
              retryableErrors: (error) => {
            ...
            if (isDuplicateError) {
              ...
              for (const commit of batch) {
                try {
                  await retry(() => tx.insert(commitsTable).values(commit), {
                    maxRetries: 2,
                    initialDelayMs: 200,
                    retryableErrors: (err) => {
                  ...
                  if (isDbTimeout(err)) {
                    logger.error(
                      "DB timeout while inserting single commit",
                      err,
                      {
                        hash: commit.hash,
                        repoId,
...
async function updateContributions(
  ...
  commits.forEach((commit) => {
    // Ensure valid date
    if (!commit.commitDate || Number.isNaN(commit.commitDate.getTime())) {
  ...
  const contributionsToUpsert = Array.from(contributionsByDate.entries()).map(
    ([dateStr, count]) => {
  ...
  for (const contribution of contributionsToUpsert) {
    try {
      ...
      if (isDuplicateError || error.message?.includes("Failed query")) {
        ...
        try {
          const _updateResult = await dbInstance
  ...
  try {
    ...
    const syncPromises = reposToSync.map(async (repo) => {
      try {
        ...
        const commitCount = await syncRepoCommits(
          userId,
          repo.id,
          githubService,
        );
    ...
    for (const result of results) {
      if (result.success) {
        ...
        errors.push(
          `Failed to sync ${(result as any).repoFullName}: ${(result as any).error}`,
        );
      }
    }
...
export async function getContributionsFromDB(
  ...
  try {
    ...
    if (isDbTimeout(error)) {
      logger.warn(
        "DB timeout in getContributionsFromDB; returning empty data placeholder.",
        {
          userId,
          days,
        },
...
export async function getTotalCommitsCount(userId: number): Promise<number> {
  try {
    ...
    if (isDbTimeout(error)) {
      logger.warn("DB timeout in getTotalCommitsCount; returning 0.", {
        userId,
      });
      return 0;
    }
...
export async function needsSync(userId: number): Promise<boolean> {
  try {
    ...
    if (isDbTimeout(error)) {
      logger.warn(
        "DB timeout in needsSync; returning true to trigger sync later.",
        { userId },
      );
      return true;
    }
