...
const SENSITIVE_ACTIVITY_FIELDS = [
  "ipAddress",
  "userAgent",
  "metadata",
] as const;
...
export async function logActivity(params: LogActivityParams): Promise<void> {
  try {
    ...
    const encryptedParams = encryptSensitiveData(
      params,
      SENSITIVE_ACTIVITY_FIELDS,
    );
  ...
  try {
    ...
    try {
      ...
      if (
        ...
        dbError.message?.includes("fingerprints")
      ) {
        console.warn("Fingerprints table does not exist. Run: pnpm db:push");
        throw new Error(
          'Database table "fingerprints" does not exist. Please run: pnpm db:push',
        );
      }
      throw dbError;
    }
