...
import type { SideBySideLine } from "./diffParser";
import {
...
const PATTERNS = {
  ...
  blockStart:
    /^\s*(?:if|else|elif|for|while|try|catch|switch|case|with)\s*[(:]/,
  blockEnd: /^\s*[}\]]/,
};
...
export function groupChanges(
  ...
  for (let i = 0; i < sideBySideLines.length; i++) {
    ...
    if (line.type === "context") {
      ...
      if (currentGroup && currentGroup.changes.length > 0) {
        ...
        for (
          let j = i + 1;
          j < Math.min(i + contextWindow * 2, sideBySideLines.length);
          j++
        ) {
      ...
      // Update group
      currentGroup.changes.push(line);
      currentGroup.endLine =
        line.left?.lineNumber || line.right?.lineNumber || currentGroup.endLine;
  ...
  for (const group of groups) {
    ...
    try {
      ...
      if (oldCode || newCode) {
        group.semanticChanges = analyzeSemanticChanges(
          oldCode,
          newCode,
          language,
        );
...
function generateGroupDescription(group: ChangeGroup): string {
  ...
  if (group.additions > 0 && group.deletions > 0) {
    ...
    parts.push(
      `Added ${group.additions} line${group.additions > 1 ? "s" : ""}`,
    );
  } else if (group.deletions > 0) {
    parts.push(
      `Removed ${group.deletions} line${group.deletions > 1 ? "s" : ""}`,
    );
  }
