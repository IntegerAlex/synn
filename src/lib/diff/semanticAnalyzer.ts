...
export interface SemanticChange {
  type: 'function-signature' | 'variable-rename' | 'initialization' | 'refactor' | 'addition' | 'deletion';
  description: string;
...
function compareASTs(
  ...
  for (const newVar of newVars) {
    const similarOldVar = oldVars.find(v => 
      v.value === newVar.value && v.name !== newVar.name
    );
  ...
  function visit(node: any) {
    ...
    if (ts.isFunctionDeclaration(node) && node.name) {
      const params = node.parameters.map(p => ({
        ...
        type: p.type ? node.getText().substring(p.type.pos - node.pos, p.type.end - node.pos) : undefined,
      }));
      ...
      node.declarationList.declarations.forEach(decl => {
        if (decl.name && ts.isIdentifier(decl.name) && decl.initializer) {
          if (ts.isArrowFunction(decl.initializer) || ts.isFunctionExpression(decl.initializer)) {
            ...
            const lineNumber = ast.getLineAndCharacterOfPosition(decl.pos).line + 1;
...
function extractVariables(ast: any): VariableInfo[] {
  ...
  function visit(node: any, scope: string = 'global') {
    if (ts && ts.isVariableDeclaration(node) && node.name && ts.isIdentifier(node.name)) {
    ...
    if (ts && ts.isFunctionDeclaration(node) && node.name) {
 ...
 */
export function extractSemanticScope(
  group: { title: string; type: string; changes: Array<{ left?: { content: string }; right?: { content: string } }> }
): string {
