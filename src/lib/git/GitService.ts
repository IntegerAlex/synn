...
import simpleGit, { type BranchSummary, type SimpleGit } from "simple-git";
...
class GitService {
    ...
    try {
      ...
      for (const line of lines) {
        if (line.match(/^[a-f0-9]{40}/)) {
          ...
          currentCommit = {
            hash: parts[0],
            lineNumber: parseInt(parts[2], 10) || ++lineNumber,
          };
  ...
  async compareBranches(base: string, compare: string): Promise<BranchCompare> {
    ...
    const [behind, ahead] = revListOutput
      .trim()
      .split("\t")
    ...
    return {
      ...
      files,
      stats: {
        filesChanged: statMatch ? parseInt(statMatch[1], 10) : files.length,
        additions: statMatch?.[2] ? parseInt(statMatch[2], 10) : 0,
