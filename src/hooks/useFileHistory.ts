...
export function useFileHistory(filepath: string | null, options?: { ref?: string; limit?: number }) {
  const repoInfo = useAppStore((state) => state.repoInfo);

  return useQuery<FileHistoryEntry[]>({
    queryKey: ['fileHistory', repoInfo?.path, filepath, options?.ref, options?.limit],
